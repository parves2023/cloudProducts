...
const ProfilePayment = () => {
    ...
    const handleApplyCoupon = async () => {
        try {
            const response = await axios.post("http://localhost:5000/validate-coupon", {
                couponCode,
                amount: 9900,
            });
            ...
            Swal.fire("Coupon Applied", `You saved $${discountAmount / 100}`, "success");
        } catch (error) {
            console.error("Error applying coupon:", error);
            Swal.fire("Invalid Coupon", error.response?.data?.message || "Failed to apply coupon", "error");
        }
    };
